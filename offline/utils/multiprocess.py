...
def expand_call(**kargs):
    ...
    使用灵活的关键字参数接收调用任务中的所有参数，并调用回调函数。

    :param kargs: Job 参数的关键字参数（包含 'func' 及其它所需的参数）
    :return: Job 执行结果
    """
    func = kargs.pop("func")
    return func(**kargs)
...
def process_jobs(jobs, task=None, num_threads=24, verbose=True):
    """
    使用mpire库进行并行处理。jobs必须包含'func'回调函数供expand_call使用。

    :param jobs: (list) 任务列表（分子）
    :param task: (str) 任务描述
    :param num_threads: (int) 并行使用的线程数（每个线程一个处理器）
    :param verbose: (bool) 是否报告异步任务进度
    :return: (list) 处理结果列表
    """
    if task is None:
