...
**注意**：切片粒度（granularity）不固定，通常在 **0.2 到 0.01** 之间（如 0.01、0.02、0.05、0.1、0.2），需从文件名动态解析。
...
**零样本边界处理**：如果边界切片的样本数严格为零（ratio=0），需要从边缘向中间方向寻找第一个样本不为零的切片作为实际边界进行分析。例如：多头边界 [0.98, 1.00] 样本为零，则检查 [0.96, 0.98]，以此类推。
