"""
Alpha #51: Momentum Threshold 2

Formula: (((((delay(close, 20) - delay(close, 10)) / 10) - ((delay(close, 10) - close) / 10)) < (-1 * 0.05)) ?
          1 : ((-1) * (close - delay(close, 1))))

Type: Price/Volume
Description: Tighter momentum threshold (0.05 vs 0.1 in alpha_049).
"""
...
def alpha_051(
    ...
    Formula: if momentum_diff < -0.05: 1 else: -1 * (close - delay(close, 1))
...
if __name__ == "__main__":
    ...
    seq_result = alpha_051(candles, sequential=True)
    single_result = alpha_051(candles, sequential=False)
