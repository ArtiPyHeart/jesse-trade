BUILDIN_FEATURES = (
    ...
    + [f"fracdiff_o_o{l}" for l in range(1, 6)]
    + [f"fracdiff_o_h{l}" for l in range(1, 6)]
    + [f"fracdiff_o_l{l}" for l in range(1, 6)]
    + [f"fracdiff_o_c{l}" for l in range(1, 6)]
    + [f"fracdiff_h_o{l}" for l in range(1, 6)]
    + [f"fracdiff_h_h{l}" for l in range(1, 6)]
    + [f"fracdiff_h_l{l}" for l in range(1, 6)]
    + [f"fracdiff_h_c{l}" for l in range(1, 6)]
    + [f"fracdiff_l_o{l}" for l in range(1, 6)]
    + [f"fracdiff_l_h{l}" for l in range(1, 6)]
    + [f"fracdiff_l_l{l}" for l in range(1, 6)]
    + [f"fracdiff_l_c{l}" for l in range(1, 6)]
    + [f"fracdiff_o_o{l}" for l in range(1, 6)]
    + [f"fracdiff_o_h{l}" for l in range(1, 6)]
    + [f"fracdiff_o_l{l}" for l in range(1, 6)]
    + [f"fracdiff_o_c{l}" for l in range(1, 6)]
    + [f"fracdiff_c_o{l}" for l in range(1, 6)]
    + [f"fracdiff_c_h{l}" for l in range(1, 6)]
    + [f"fracdiff_c_l{l}" for l in range(1, 6)]
    + [f"fracdiff_c_c{l}" for l in range(1, 6)]
