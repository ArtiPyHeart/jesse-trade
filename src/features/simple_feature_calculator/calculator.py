...
class SimpleFeatureCalculator:
    ...
    def __init__(self, registry: Optional[SimpleFeatureRegistry] = None):
        """
        初始化计算器
    ...
    def _compute_feature(self, feature_name: str) -> np.ndarray:
        ...
        if transforms:
            # 验证转换输入
            self.validator.validate_transform_input(base_value, transforms[0], base_name)
            # 应用转换
    ...
    def _compute_base_feature(self, feature_name: str, force_sequential: bool) -> np.ndarray:
        """
        计算基础特征（不含转换）
    ...
    def _parse_feature_name(self, feature_name: str) -> Tuple[str, List[str], Optional[int]]:
        """
        解析特征名称
