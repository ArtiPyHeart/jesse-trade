...
def test_basic_features_consistency(candles):
    ...
    for feature_name in test_features:
        try:
            ...
            # 比较结果
            if np.allclose(
                original_value, new_value, rtol=1e-5, atol=1e-8, equal_nan=True
            ):
...
def test_transformation_consistency(candles):
    ...
    # 计算原始特征
    original_features = feature_bundle(
        test_candles, sequential=True, lightweighted=True
    )
