"""
Pipeline Model Train Individual - 每个模型独立 FeaturePipeline

流程：
1. 读取 feature_selection_results.csv
2. 对每个 (lag, pred_next, label_type):
   - 读取该行的 selected_features
   - 构建独立的 FeaturePipeline (启用降维) → 一阶特征计算 + SSM 训练/transform + ARDVAE 降维
   - 保存 FeaturePipeline 到 MODEL_DIR/<model_name>/（与模型同名）
   - 生成标签 → 对齐 → Optuna 调参 → 训练 LightGBM → 保存

与 pipeline_model_train.py 的区别：
- 每个模型有独立的 FeaturePipeline（而非全局共用）
- Pipeline 保存目录: <model_name>/（如 c_L4_N1/）
"""
...
RESULTS_FILE = "model_train_individual_results.csv"
...
class ModelTrainTracker:
    ...
    def save_result(
        ...
        """保存单个实验结果"""
        model_name = get_model_name(log_return_lag, pred_next, label_type)
    ...
    def get_pending_tasks(self, df_selection: pd.DataFrame) -> list[tuple]:
        ...
        从 feature_selection_results.csv 中提取所有 (lag, pred_next, label_type, selected_features) 组合
        """
        pending = []
        for _, row in df_selection.iterrows():
            ...
            if not self.is_completed(lag, pred_next, label_type):
                pending.append((lag, pred_next, label_type, selected_features))
    ...
    def print_summary(self):
        ...
        for label_type in ["hard", "direction"]:
            ...
            if not type_df.empty:
                ...
                print(f"  - 输入特征: {int(best_row['n_features_input'])}")
                print(f"  - 降维后特征: {int(best_row['n_features_reduced'])}")
...
def build_individual_pipeline(
    ...
    """
    构建单个模型的独立 FeaturePipeline（启用降维）

    Args:
        candles: K线数据
        selected_features: 该模型的特征列表
    ...
    """
    logger.info(
        f"[{model_name}] 构建独立 Pipeline，输入特征: {len(selected_features)} 个"
    )
...
def train_model_with_individual_pipeline(
    candles: np.ndarray,
    log_return_lag: int,
    pred_next: int,
    label_type: str,
    selected_features: list[str],
) -> tuple[FeaturePipeline, dict, float, lgb.Booster, int]:
    """
    训练单个模型（含独立 Pipeline）：
    1. 构建独立 FeaturePipeline
    2. 生成标签 → 对齐 → Optuna 调参 → 全量训练
    ...
    Returns:
        (pipeline, best_params, best_score, model, n_reduced_features)
    """
    is_regression = label_type == "direction"
    model_name = get_model_name(log_return_lag, pred_next, label_type)

    logger.info(
        f"[{model_name}] 开始训练 - log_return_lag={log_return_lag}, "
    ...
    if is_regression:
        ...
        logger.info(
            f"[{model_name}] 标签统计: 均值={np.mean(valid_labels):.6f}, "
    ...
    return pipeline, best_params, best_score, model, n_reduced_features
...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info("Pipeline Model Train Individual - 每个模型独立 FeaturePipeline")
    logger.info("=" * 60)
    ...
    if pending_tasks:
        logger.info("\n待完成任务列表:")
        for i, (lag, pred_next, label_type, features) in enumerate(
            pending_tasks[:5], 1
        ):
            model_name = get_model_name(lag, pred_next, label_type)
            logger.info(f"  {i}. {model_name} ({label_type}) - {len(features)} 特征")
    ...
    # 4. 遍历训练每个模型
    logger.info("\n" + "=" * 60)
    logger.info("开始模型训练主循环（每个模型独立 Pipeline）")
    logger.info("=" * 60)

    for task_idx, (lag, pred_next, label_type, selected_features) in enumerate(
        pending_tasks, 1
    ):
        model_name = get_model_name(lag, pred_next, label_type)
        ...
        try:
            ...
            pipeline, best_params, best_score, model, n_reduced_features = (
                train_model_with_individual_pipeline(
                    candles,
                    lag,
                    pred_next,
                    label_type,
            ...
            model_dir.mkdir(parents=True, exist_ok=True)
            model_save_path = model_dir / f"model_{model_name}.txt"
            model.save_model(model_save_path.resolve().as_posix())
            logger.info(f"[{model_name}] 模型已保存到: {model_save_path}")
            ...
            tracker.save_result(
                ...
                n_features_reduced=n_reduced_features,
                duration=duration,
                status="completed",
            )
            ...
            logger.info(f"  - 输入特征: {len(selected_features)}")
            logger.info(f"  - 降维后特征: {n_reduced_features}")
