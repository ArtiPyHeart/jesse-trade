...
def estimate_dollar_bar_threshold(
    candles: np.ndarray, num_minutes: int, with_sign: bool = with_sign
) -> float:
    """
    ...
    if with_sign:
        ...
        dollar_volume = dollar_volume * sign
    ...
    for _ in range(max_iterations):
        ...
        bars = build_dollar_bar(candles, mid_threshold, with_sign=with_sign)
...
def build_dollar_bar(
    ...
    for i in range(n - 1, -1, -1):
        ...
        if with_sign:
            sign = 1.0 if (c - o) > 0 else -1.0
        else:
            sign = 1.0

        dv = c * v * sign  # 本根K线的 dollar volume
        bar_timestamp = ts
        ...
        # 达到阈值后把这根 bar 放进结果
        if abs(current_dollar_volume) > threshold:
...
class DollarBarContainer:
    ...
    def _load_initial_candles(self, candles: np.ndarray):
        ...
        initial_bars = build_dollar_bar(
            candles, self.threshold, max_bars=self.max_bars, with_sign=self.with_sign
        )
    ...
    def _update_with_candle(self, candle: np.ndarray):
        ...
        # 检查是否达到阈值
        if abs(self.current_dollar_volume) > self.threshold:
            # 构建新的完整bar
            new_bar = np.array(
                [
                    self._bar_timestamp,
