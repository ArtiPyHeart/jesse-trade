...
def _cwt_pywavelets_reference(
    signal, scales, wavelet="cmor1.5-1.0", sampling_period=1.0
):
    """
    ...
    coef_complex, freqs = pywt.cwt(
        signal, scales, wavelet, sampling_period=sampling_period
    )

    # 转置以匹配 Rust 实现的形状 (signal_len, num_scales)
    coef_complex = coef_complex.T

    # 转换为 dB 尺度（与 Rust 实现一致：log10）
    epsilon = 1e-12
    coef_db = np.log10(np.abs(coef_complex) + epsilon)

    return coef_db, freqs


@pytest.mark.skipif(
    not HAS_RUST or not HAS_PYWT, reason="Rust or PyWavelets not available"
)
def test_cwt_basic_comparison():
    ...
    # 验证形状一致
    assert coef_pywt.shape == coef_rust.shape, (
        f"系数形状不一致: {coef_pywt.shape} vs {coef_rust.shape}"
    )
    assert freqs_pywt.shape == freqs_rust.shape, (
        f"频率形状不一致: {freqs_pywt.shape} vs {freqs_rust.shape}"
    )
    ...
    print(
        f"    频率范围 (PyWavelets): [{np.min(freqs_pywt):.4f}, {np.max(freqs_pywt):.4f}]"
    )
    print(
        f"    频率范围 (Rust):       [{np.min(freqs_rust):.4f}, {np.max(freqs_rust):.4f}]"
    )
    ...
    print(
        f"    系数范围 (PyWavelets): [{np.min(coef_pywt):.2f}, {np.max(coef_pywt):.2f}] dB"
    )
    print(
        f"    系数范围 (Rust):       [{np.min(coef_rust):.2f}, {np.max(coef_rust):.2f}] dB"
    )
...
def test_cwt_different_wavelets():
    ...
    for wavelet in wavelets:
        ...
        # PyWavelets 实现
        coef_pywt, _ = _cwt_pywavelets_reference(
            signal, scales, wavelet=wavelet, sampling_period=1.0
        )
        ...
        assert coef_mean_diff < 0.5, (
            f"小波 '{wavelet}' 平均误差过大: {coef_mean_diff:.2e} dB"
        )
...
def test_cwt_different_scales():
    ...
    for case in scale_ranges:
        scales = np.logspace(
            np.log2(case["min"]), np.log2(case["max"]), num=case["num"], base=2
        )
        print(
        ...
        assert coef_mean_diff < 1.0, (
            f"尺度范围 '{case['name']}' 平均误差过大: {coef_mean_diff:.2e} dB"
        )
...
def test_cwt_rust_specific_features():
    ...
    # 但不应该产生巨大差异
    assert center_mean_diff < 10.0, (
        f"填充导致的中心区域差异过大: {center_mean_diff:.2e} dB"
    )
